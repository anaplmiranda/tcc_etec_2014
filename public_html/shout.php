...
{
	//connect to mysql db
	$sql_con = mysqli_connect($hostname, $usuario, $senha,$banco)or die('could not connect to database');
	...
	if(isset($_POST["message"]) &&  strlen($_POST["message"])>0)
	{
		...
		if(mysqli_query($sql_con,"INSERT INTO shout_box(id_user, message, ip_address) value('$id_user','$message','$user_ip')"))
		{
			$msg_time = date('h:i A M d',time()); // current time
			echo '<div class="shout_msg"><time>'.$msg_time.'</time><span class="username">'.$usuario.'</span><span class="message">'.$message.'</span></div>';
		}
